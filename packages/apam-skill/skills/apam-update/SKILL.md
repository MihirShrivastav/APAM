...
Call `apam_status` with **no arguments**. Copy the 16-char hex string from the `Project:` line exactly.

If you already called `apam_status` earlier this session and have the project_id, you can skip this call — but only if you are certain the value is correct.
...
- Any new facts about the project structure that weren't in memory before

### 3. Update L1 atoms

L1 is not immutable — atoms must stay accurate. For each L1 atom:
...
- The stack changed (new dependency added, library swapped) → update the stack atom
- An old constraint was lifted or a new one added → update the constraint atom

To update: call `apam_pin` with the corrected content. The upsert deduplicates on type+content+scope — so write the full corrected fact, not just the delta.
...
- `project_id`: the hex you copied in step 1
- `session_start` / `session_end`: approximate ISO timestamps for this session
- `git_branch`: current branch (if you know it)
- `summary`: 2–4 sentences — what was done and why
- `decisions`: key choices made
- `files_touched`: files significantly changed
- `problems_solved`: bugs or blockers cleared
- `patterns_observed`: anything recurring noted

Write multiple episodes if multiple distinct chunks of work were done.

### 6. Report what was written

Briefly tell the user what was saved: how many L1 atoms pinned, which L3 records updated, episode written.
